...
nearlyBuyCoins=''
nearlySellCoins=''
api_key='Get your API key from BINANCE' #TODO
api_secret='Get your SECRET key from BINANCE' #TODO
...
def marginBuySellByMarketPriceAction(firstCoinNameForBuyOrSellParameter,secondCoinNameForBuyOrSellParameter,usdtAmount,exhangeQuantity,buyOrSell):
    ...
    if buyOrSell=='BUY': #alış yapmak için gerekiyorsa önce transfer et
        ...
        try :
            repayAmount = client.get_margin_loan_details(asset=firstCoinNameForBuyOrSellParameter)['rows'][0]['principal'] # Kredi ödemesi Eskisi geliyor
...
def commonBuySellByMarketPriceAction(firstCoinNameForBuyOrSellParameter,secondCoinNameForBuyOrSellParameter,quantity,buyOrSell,myTrade,buyedByRobot,buyingStrategy):
    ...
    quantityFromExchange=getRemainingAsset(firstCoinNameForBuyOrSellParameter,secondCoinNameForBuyOrSellParameter,myTrade)
    if buyOrSell=='SELL':
        if myTrade is None: 
